...
from parqueryd.worker import DownloaderNode, WorkerNode
...
def taxi_df():
    taxi_df = pd.read_csv(
        "/srv/datasets/yellow_tripdata_2016-01.csv", parse_dates=["tpep_pickup_datetime", "tpep_dropoff_datetime"]
    )
    yield taxi_df
...
def rpc():
    ...
    worker = WorkerNode(redis_url=TEST_REDIS, loglevel=logging.DEBUG, restart_check=False)
...
def shards(taxi_df):
    ...
    for idx in range(0, len(taxi_df), step):
        ...
        shard_file = os.path.join(parqueryd.config.DEFAULT_DATA_DIR, f"yellow_tripdata_2016-01-{count}.parquet")
...
def test_rpc_info(rpc):
    ...
    worker_node = [v for v in workers.values() if v["workertype"] == "calc"][0]

    node = result["node"]
    assert node == worker_node["node"]

    downloader_node = [v for v in workers.values() if v["workertype"] == "download"][0]
