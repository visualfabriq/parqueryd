...
class LocalS3Downloader(DownloaderNode):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._conn = boto3.resource(
            "s3", region_name=AWS_REGION, aws_access_key_id=FAKE_ACCESS_KEY, aws_secret_access_key=FAKE_SECRET_KEY
        )
    ...
    def _get_transport_params(self):
        return {"resource_kwargs": {"endpoint_url": "http://localstack:4572"}}
...
def test_downloader(redis_server, downloader, tmpdir):
    ...
    data_df = pd.DataFrame(data=np.random.rand(100, 10), columns=[f"col_{i + 1}" for i in range(10)])
    ...
    with clean_bucket(s3_conn, "parquet") as bucket:
        bucket.put_object(Key="test.parquet", Body=open(local_parquet, "rb"))  # noqa: SIM115
        ...
        progress_slot = f"{time.time() - 60}_{-1}"
        node_filename_slot = "{}_{}".format(socket.gethostname(), "s3://parquet/test.parquet")
        ticket = str(uuid4())
