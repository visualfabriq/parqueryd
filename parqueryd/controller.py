...
from parqueryd.util import bind_to_random_port, get_my_ip


def deserialize_pa_table_compat(data: Any) -> pa.Table:
    """Deserialize PyArrow table with backward compatibility.

    Tries bytes deserialization first (new format), falls back to base64 (old format).
    """
    try:
        return deserialize_pa_table_bytes(data)
    except Exception:
        return deserialize_pa_table_base64(data)


POLLING_TIMEOUT = (
    500  # timeout in ms : how long to wait for network poll, this also affects frequency of seeing new nodes
)
...
MIN_CALCWORKER_COUNT = (
    0.1  # percentage of workers that should ONLY do calcs and never do downloads to prevent download swamping
)
RUNFILES_LOCATION = "/srv/"  # Location to write a .pid and .address file


class ControllerNode:
    """Controller node that coordinates worker nodes and handles RPC requests."""
    ...
    def __init__(
        ...
        self.address: bytes = ens_bytes(
            bind_to_random_port(self.socket, f"tcp://{get_my_ip()}", min_port=14300, max_port=14399, max_tries=100)
        )
        ...
        self.logger: logging.Logger = parqueryd.logger.getChild("controller").getChild(ens_unicode(self.address))
        self.logger.setLevel(loglevel)

        self.msg_count_in: int = 0
        self.rpc_results: list[Message] = []  # buffer of results that are ready to be returned to callers
        self.rpc_segments: dict[
            str, dict[str, Any]
        ] = {}  # Certain RPC calls get split and divided over workers, this dict tracks the original RPCs
        self.worker_map: dict[
            bytes, dict[str, Any]
        ] = {}  # maintain a list of connected workers TODO get rid of unresponsive ones...
        self.files_map: dict[str, set[bytes]] = {}  # shows on which workers a file is available on
        self.worker_out_messages: dict[Any, list[Message]] = {
            None: []
        }  # A dict of buffers, used to round-robin based on message affinity
        self.worker_out_messages_sequence: list[Any] = [None]  # used to round-robin the outgoing messages
        self.is_running: bool = True
        self.last_heartbeat: float = 0
        self.others: dict[bytes, dict[str, Any]] = {}  # A dict of other Controllers running on other DQE nodes
        self.start_time: float = time.time()
        self.azure_conn_string: str | None = azure_conn_string

    def send(self, addr: bytes, msg_buf: bytes, is_rpc: bool = False) -> None:
        """Send message to a worker or controller.

        Args:
            addr: Address to send to
            msg_buf: Message buffer to send
        ...
        try:
            if addr == self.address:
                ...
                return
            tmp = [addr, b"", msg_buf] if is_rpc else [addr, msg_buf]
            self.socket.send_multipart(tmp)
        except zmq.ZMQError as ze:
            self.logger.warning(f"Problem with {addr}: {ze}")

    def connect_to_others(self) -> None:
        """Connect to other controller nodes via Redis discovery."""
        ...
        for x in all_servers:
            if x not in self.others:
                ...
                try:
                    ...
                    self.logger.critical(f"Removing {x} due to {e}")
        ...
        for x in list(self.others.keys()):  # iterate over a copy of keys so we can remove entries
            if x not in all_servers:
                self.logger.critical(f"Disconnecting from {x}")
                try:
                    del self.others[x]
    ...
    def heartbeat(self) -> None:
        """Send heartbeat and connect to other controllers."""
        if time.time() - self.last_heartbeat > HEARTBEAT_INTERVAL:
            self.connect_to_others()
            self.last_heartbeat = time.time()

    def find_free_worker(self, needs_local: bool = False, filename: str | None = None) -> bytes | None:
        """Find a free worker to send a task to.

        Args:
            needs_local: Whether worker must be on same node
            filename: If specified, worker must have this file

        Returns:
            Worker ID or None if no free worker found
        """
        # Pick a random worker_id to send a message, TODO add some kind of load-balancing
        free_workers: list[bytes] = []
        free_local_workers: list[bytes] = []
    ...
    def process_sink_results(self) -> None:
        """Process results ready to be returned to RPC callers."""
        while self.rpc_results:
            ...
            if "parent_token" in msg:
                ...
                if parent_token not in self.rpc_segments:
                    logging.debug(f"Orphaned msg segment {parent_token} probably an error was raised elsewhere")
                    continue
                original_rpc = self.rpc_segments.get(parent_token)

                if isinstance(msg, ErrorMessage):
                    self.logger.debug("Error message {}".format(msg.get("payload")))
                ...
                if msg.get("data"):
                    result_table = deserialize_pa_table_compat(msg["data"])
                ...
                logging.debug(
                    "Received result for token "
                    + ens_unicode(parent_token)
                    + " for file "
                    + ens_unicode(filename)
                    + " ("
                    + str(len(original_rpc["results"]))
                    + "/"
                    + str(len(original_rpc["filenames"]))
                    + ")"
                )
                ...
                if len(original_rpc["results"]) == len(original_rpc["filenames"]):
                    ...
                    pa_tables = [
                        result_table for result_table in original_rpc["results"].values() if result_table is not None
                    ]

                    logging.debug(
                        "Last result received for token "
                        + ens_unicode(parent_token)
                    ...
                    if len(pa_tables) == 0:
                        ...
                        msg["data"] = serialize_pa_table_bytes(pa_tables[0])
                    else:
                        msg["data"] = serialize_pa_table_bytes(pa.concat_tables(pa_tables))
            ...
            if "data" in msg:
                ...
                try:
                    ...
                    self.logger.debug("Bytes error with " + str(msg))
            self.logger.debug("RPC Msg handled: {}".format(msg.get("payload", "?")))
            self.logger.debug(f"Pending results: {str(len(self.rpc_segments))}")

    def handle_out(self) -> None:
        """Handle outgoing messages to workers."""
        ...
        try:
            ...
            self.logger.debug(f"Bytes error with {msg}")

    def handle_in(self) -> None:
        """Handle incoming messages from workers and RPC clients."""
        self.msg_count_in += 1
        data = self.socket.recv_multipart()
        binary: bytes | None = None
        sender: bytes | None = None  # initialise outside for edge cases
        if len(data) == 3:
    ...
    def handle_peer(self, sender: bytes, msg: Message) -> None:
        """Handle messages from peer controller nodes.
        ...
        if msg.isa("loglevel"):
            ...
            self.logger.debug(f"Got a msg but don't know what to do with it {msg}")

    def handle_worker(self, worker_id: bytes, msg: Message) -> None:
    ...
    def handle_rpc(self, sender: bytes, msg: Message) -> None:
        """Handle RPC requests from clients.

        Args:
            sender: Sender address
            msg: RPC message to handle
        """
        ...
        result: Any = "Sorry, I don't understand you"
        if msg.isa("ping"):
            result = "pong"
            ...
            if "function" not in kwargs:
                ...
                self.worker_out_messages[None].append(msg.copy())
                result = None if kwargs.get("wait", False) else "OK, {} dispatched".format(kwargs["function"])
    ...
    def setup_download(self, msg: Message) -> str | None:
        """Setup a download ticket for files.

        Args:
            msg: Message with download request

        Returns:
            Ticket ID or None if waiting, or error message
        """
        args, kwargs = msg.get_args_kwargs()
        ...
        if self.azure_conn_string:
            filenames = [f"azure://{bucket}/{filename}" for filename in filenames]
        else:
            filenames = [f"s3://{bucket}/{filename}" for filename in filenames]
        ...
        for filename in filenames:
            ...
            for node in nodes:
                ...
                progress_slot = f"{time.time() - 60}_{-1}"  # give the slot a timestamp of now -1 minute so we can see when it was created
                node_filename_slot = f"{node}_{filename}"
                self.redis_server.hset(
                    parqueryd.config.REDIS_TICKET_KEY_PREFIX + ticket, node_filename_slot, progress_slot
                )
    ...
    def handle_calc_message(self, msg: Message) -> str | None:
        """Handle calculation message by splitting it across workers.

        Args:
            msg: Calculation message to handle
        ...
        if len(args) != 4:
            return (
                "Error, No correct args given, expecting: "
        ...
        rpc_segment = {"msg": msg, "results": {}, "filenames": dict.fromkeys(filenames)}
    ...
    def killall(self) -> str:
        """Kill all workers and controllers.
    ...
    def get_info(self) -> dict[str, Any]:
        ...
        Returns:
            Dictionary with controller status and statistics
        """
        data = {
    ...
    def remove_worker(self, worker_id: bytes) -> None:
        """Remove a worker from the worker map.
    ...
    def free_dead_workers(self) -> None:
        """Remove workers that haven't been seen in DEAD_WORKER_TIMEOUT seconds."""
        now = time.time()
        for worker_id, worker in self.worker_map.copy().items():
    ...
    def go(self) -> None:
        ...
        while self.is_running:
            try:
                ...
                self.logger.exception(f"Exception {traceback.format_exc()}")

        self.logger.info("Stopping")
        for x in (
            os.path.join(RUNFILES_LOCATION, "parqueryd_controller.pid"),
