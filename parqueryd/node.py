...
from parqueryd.worker import DownloaderNode, MoveparquetNode, WorkerNode

NODE_TYPES = ["controller", "worker", "downloader", "moveparquet"]


def main() -> None:
    """Main entry point for parqueryd node startup."""
    parser = argparse.ArgumentParser(description="Start parqueryd worker.")
    parser.add_argument("worker_type", choices=NODE_TYPES, help="Type of node to run")
    parser.add_argument(
        "--data_dir", default=parqueryd.config.DEFAULT_DATA_DIR, help="Local directory worker nodes will read from"
    )
    ...
    azure_conn_string: str | None = config.get("azure_conn_string", None)
    sentry_dsn: str | None = config.get("sentry_dsn", None)
    ...
    if sentry_dsn is not None:
        sentry_sdk.init(
            ...
            environment=f"legacy-parqueryd-{environment}",
            release=str(parqueryd.__version__),
        )

    node: ControllerNode | WorkerNode | DownloaderNode | MoveparquetNode
